...
from PyQt6.QtWidgets import (
    QWidget, QVBoxLayout, QCheckBox, QTableWidget,
    ...
    def run(self):
        ...
        try:
            for root, dirs, files in os.walk(self.start_path):
                ...
                # 2. Search Files (filename + content)
                for f in files:
                    if not self.is_running:
                        break
...
class FileTableWidget(QWidget):
    ...
    def load_files(self, path):
        ...
        try:
            with os.scandir(path) as entries:
                items = [entry for entry in entries if not entry.name.startswith('.')]
            ...
            for row, entry in enumerate(items):
                ...
                try:
                    ...
                    date_item = QTableWidgetItem(mod_time.strftime("%d/%m/%Y"))
                except Exception:
    ...
    def show_context_menu(self, position):
        ...
        if action == action_new:
            self.perform_action("new")
        elif action == action_cut:
            self.perform_action("cut")
        elif action == action_copy:
            self.perform_action("copy")
        elif action == action_paste:
            self.perform_action("paste")
        elif action == action_rename:
            self.perform_action("rename")
        elif action == action_share:
            self.perform_action("share")
