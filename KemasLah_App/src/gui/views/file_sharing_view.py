    ...
    def __init__(self, file_path, cloud_file_name, current_user_email, is_owner=False, parent=None):
        ...
        try:
            ...
            self.content_view.setText(
                "This is a non-text file (e.g., Image, PDF, Document).\n\n"
...
class ExpiryUpdateDialog(QDialog):
    def __init__(self, current_expiry, parent=None):
        ...
        if current_expiry and str(current_expiry).strip() != "None" and str(current_expiry).strip() != "":
            ...
            qt_date = QDateTime.fromString(
                str(current_expiry).replace("⚠️ EXPIRED (", "").replace(")", "").strip(),
                "yyyy-MM-dd HH:mm:ss"
            )
...
class FileSharingView(QWidget):
    def __init__(self, current_user_email):
        ...
        self.sync_btn.setStyleSheet("""
            ...
            QPushButton:disabled { background-color: #4A5568; color: #CBD5E0; }
        """)
        ...
        # --- Changed to 8 columns and added 'Size' ---
        self.table = QTableWidget(0, 8)
        self.table.setHorizontalHeaderLabels([
            "Direction", "File Name", "Size", "Shared With / By",
            "Role", "Expiration", "Shared On", "Action"
        ])
    ...
    def _set_busy_ui(self, is_busy):
        self.sync_btn.setEnabled(not is_busy)
    ...
    def sync_files(self):
        ...
        self._show_overlay("Syncing cloud files...")
        ...
        try:
            ...
            for row in rows:
                ...
                if self.overlay:
                    self.overlay.label.setText(f"Syncing: {file_name}")
                    QApplication.processEvents()
                ...
                if cloud_comment:
                    if (not os.path.exists(local_comment_path) or
                            os.path.getmtime(local_comment_path) < cloud_comment.uploadDate.replace(tzinfo=timezone.utc).timestamp()):
    ...
    def load_shared_files(self):
        self._show_overlay("Loading shared files...")
        self._set_busy_ui(True)
        ...
        try:
            ...
            for row_data in rows:
                ...
                if self.overlay:
                    self.overlay.label.setText(f"Loading: {file_name}")
                    QApplication.processEvents()
                ...
                self.table.setItem(row_idx, 2, create_readonly_item(file_size))
                ...
                if direction == "Received":
                    ...
                    if expiry and str(expiry).strip() != "None":
                        if ext_status == "Pending":
                            ...
                            req_btn.setStyleSheet(
                                "background-color: #DD6B20; color: white; border-radius: 4px; padding: 4px; font-size: 11px; font-weight: bold;"
                            )
                            if is_expired:
                                req_btn.setStyleSheet(
                                    "background-color: #E53E3E; color: white; border-radius: 4px; padding: 4px; font-size: 11px; font-weight: bold;"
                                )
                    ...
                    if ext_status == "Pending":
                        ...
                        expiry_btn.setStyleSheet(
                            "background-color: #D69E2E; color: black; font-weight: bold; border-radius: 4px; padding: 4px;"
                        )
                        expiry_btn.clicked.connect(
                            lambda _, fp=file_path, te=recipient, rd=req_date, ce=expiry:
                            self.open_review_dialog(fp, te, rd, ce)
                        )
                    else:
                        expiry_btn.setStyleSheet("""
                        ...
                        if is_expired:
                            expiry_btn.setStyleSheet(
                                "background-color: #742A2A; color: white; border: 1px solid #FC8181; border-radius: 4px; padding: 4px; text-align: left;"
                            )
                        expiry_btn.clicked.connect(
                            lambda _, fp=file_path, te=recipient, cr=role, ce=expiry:
                            self.open_expiry_dialog(fp, te, cr, ce)
                        )
            ...
            QMessageBox.critical(self, "Load Error", f"Failed to load shared files:\n{e}")
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if request_extension(file_path, owner_email, self.user_email, new_date):
                QMessageBox.information(
                    self,
                    "Request Sent",
                    f"Your request to extend access until {new_date} has been sent to the owner."
                )
                self.load_shared_files()
            else:
