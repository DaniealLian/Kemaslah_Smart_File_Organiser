...
class SmartFileManager(QMainWindow):
    ...
    def perform_ai_deep_search(self, ai_query):
        ...
        # File types that are known to have NO readable text content
        UNSUPPORTED_TYPES = {
    ...
    def handle_smart_organise(self):
        ...
        # Pre-check: warn about any directly selected files that can't be read
        for fp in files_selected:
            fname = os.path.basename(fp)
            ext = fname.lower().split('.')[-1] if '.' in fname else ''
            if ext in UNSUPPORTED_TYPES_ORG:
                unsupported_organise_files.append(fname)
        files_selected = [fp for fp in files_selected if os.path.basename(fp) not in unsupported_organise_files]
        ...
        try:
            if len(files_selected) == 1 and not folders_selected:
                # --- SINGLE FILE: Extract top keyword from content to name a new folder ---
            ...
            if folders_selected and files_selected:
                ...
                if len(valid_files) >= 2:
                    ...
                    # Can't cluster more groups than files
                    ...
                    # Guard: SVM needs at least 2 distinct classes
                    unique_clusters = len(set(cluster_labels))
                    if unique_clusters < 2:
                        progress.close()
                        QMessageBox.information(self, "Smart Organise",
                            "These files are too similar for the AI to separate into different groups.\n\n"
    ...
    def handle_satisfaction_check(self, results, metrics, history, current_path):
        ...
        if history:
            ...
            # Parse and display metrics lines nicely
            ...
            # Question label at bottom of metrics
...
class KemaslahApp(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle("Kemaslah - Smart File Manager")
        ...
        # Use setFixedSize only for login — but allow resize after login
        self.setMinimumSize(0, 0)
        self.setMaximumSize(16777215, 16777215)  # Remove any fixed size cap
    ...
    def on_login_success(self, user_data):
        ...
        self.setMaximumSize(16777215, 16777215)  # Ensure no max cap
        self.showMaximized()
