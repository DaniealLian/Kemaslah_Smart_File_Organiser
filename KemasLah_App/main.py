...
class SmartFileManager(QMainWindow):
    """Enterprise File Manager: AI Search, Smart Organise & File Sharing"""
    ...
    def __init__(self, user_data=None, auth_window=None):
        ...
        self.stack.addWidget(self.sharing_view)     # Index 4
    ...
    def open_share_dialog(self, file_path):
        """Triggers the sharing dialog with RBAC and Expiration controls."""
        is_guest = (self.user_data.get("username") == "Guest User")
        dialog = ShareFileDialog(file_path=file_path,
    ...
    # -------------------------------------------------------------------------
    # --- SEARCH: Debounced Keyword + AI Deep Search ---
    # -------------------------------------------------------------------------
    def handle_search(self, query):
        """
        Two-stage search:
          1. Immediately filters the file table by filename (fast).
          2. After 800ms of inactivity, triggers an AI content scan (slow).
        """
        self.current_search_query = query
        if self.stack.currentWidget() != self.files_view:
            self.switch_view("files")

        # Stage 1: instant filename filter
        self.files_view.file_table.filter_files(query)

        # Stage 2: debounced deep content scan (only fires if query >= 3 chars)
        self.search_timer.start(800)

    def perform_search(self):
        """Called by the timer after debounce delay."""
    ...
    def perform_ai_deep_search(self, ai_query):
        ...
        files_in_dir = [
            os.path.join(current_path, f) for f in os.listdir(current_path)
        ...
        if not files_in_dir:
            QMessageBox.information(
                ...
                f"Please navigate to the exact folder where your text file is saved!"
            )
            return

        progress = QProgressDialog(
            f"Deep scanning inside {len(files_in_dir)} documents...", "Cancel", 0, len(files_in_dir), self
        )
        ...
        if not valid_files:
            QMessageBox.information(
                self, "Content Scanner",
                f"Scanned {len(files_in_dir)} files, but none contained readable text.\n\n"
                f"Make sure your text file was actually saved (Ctrl+S)!"
            )
            return
        ...
        try:
            ...
            if not found_matches:
                QMessageBox.information(
                    self, "Content Scanner",
                    f"Scanned {len(valid_files)} files in '{os.path.basename(current_path)}'.\n\n"
                    f"Could not find any text matching '{ai_query}'.\n\n"
                    f"Are you 100% sure you moved the file into THIS exact folder?"
                )
                return
            ...
            instruction_lbl = QLabel(
                ...
                f"<i>(Note: Files that don't have this word in their title are hidden behind this box)</i>"
            )
    ...
    # -------------------------------------------------------------------------
    # --- SMART ORGANISE: RF + SVM Ensemble Sorting ---
    # -------------------------------------------------------------------------
        ...
        try:
            ...
            if len(folders_selected) >= 2 and not files_selected:
                ...
                vectorizer = TfidfVectorizer(
                    stop_words='english', max_features=2500, ngram_range=(1, 2), sublinear_tf=True
                )
                ...
                if len(folder_profiles) >= 2:
                    ...
                    if max_sim > -1:
                        ...
                        try:
                            ...
                            results_message += (
                                f"📁 Force Merged Folders: [{os.path.basename(child)}] ➡️ [{os.path.basename(parent)}]\n"
            ...
            # --- Mode 2: Sort files into existing folders (supervised) ---
            if folders_selected and files_selected:
                ...
                if valid_unsorted_files:
                    progress = QProgressDialog(
                        "Analyzing Destinations...", "Cancel", 0, len(valid_unsorted_files), self
                    )
                    progress.setMinimumDuration(0)
                    progress.setWindowModality(Qt.WindowModality.WindowModal)
                    ...
                    vectorizer = TfidfVectorizer(
                        stop_words='english', max_features=5000, ngram_range=(1, 3),
                        min_df=1, max_df=0.80, sublinear_tf=True
                    )
                    X_all = vectorizer.fit_transform(training_texts)

                    rf_model = RandomForestClassifier(
                        n_estimators=300, criterion='entropy', class_weight='balanced', random_state=42
                    )
                    svm_model_sup = SVC(
                        kernel='linear', C=3.0, class_weight='balanced', probability=True, random_state=42
                    )
            ...
            # --- Mode 3: Cluster files only (unsupervised) ---
                ...
                for folder in folders_selected:
                    for root, dirs, files in os.walk(folder):
                        for file in files:
                            pool_of_files.append(os.path.join(root, file))
                ...
                if len(valid_files) >= 2:
                    progress = QProgressDialog(
                        "Extracting boundaries via Soft Voting...", "Cancel", 0, len(valid_files), self
                    )
                    progress.setMinimumDuration(0)
                    progress.setWindowModality(Qt.WindowModality.WindowModal)
                    ...
                    vectorizer = TfidfVectorizer(
                        ...
                        min_df=1, max_df=0.90, sublinear_tf=True
                    )
                    ...
                    svm_model_unsup = SVC(
                        kernel='linear', C=2.0, class_weight='balanced', probability=True, random_state=42
                    )
    ...
    def handle_satisfaction_check(self, results, metrics, history, current_path):
        ...
        if history:
            ...
            instruction_lbl = QLabel(
                "<b>Review the AI sorting results below. Are you satisfied with this organization?</b><br>"
                "Selecting 'Undo' will return all items to their original place."
            )
            layout.addWidget(instruction_lbl)
            ...
            if dialog.exec() == QDialog.DialogCode.Rejected:
                ...
                QMessageBox.information(
                    self, "Restored",
                    "All files and folders have been returned to their original positions."
                )
                if hasattr(self.files_view, 'file_table'):
                    self.files_view.file_table.load_files(current_path)
    ...
    def on_home_folder_opened(self, path):
        """Triggered when a user double-clicks a folder in the Home tab."""
