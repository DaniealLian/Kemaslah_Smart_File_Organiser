...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QHBoxLayout, 
                             ...
                             QProgressDialog, QTextEdit, QScrollArea, QFrame)
...
class SmartFileManager(QMainWindow):
    ...
    def perform_ai_deep_search(self, ai_query):
        """Scans document contents with Explicit Error Reporting and CNN Vision."""
        ...
        # ── PART A: Text document search ─────────
        if text_files:
            progress = QProgressDialog(f"Deep scanning inside {len(text_files)} documents...", "Cancel", 0, len(text_files), self)
            progress.setMinimumDuration(500) 
            progress.setWindowModality(Qt.WindowModality.WindowModal)

            # File types that have text content and are supported
            SUPPORTED_TEXT_TYPES = {
                'pdf', 'docx', 'txt', 'md', 'csv', 'py', 'json',
                'xlsx', 'xls', 'pptx', 'rtf', 'xml', 'html', 'htm',
                'yaml', 'yml', 'toml', 'ini', 'log'
            }
            # File types that are known to have NO readable text content (excluding media handled by CNN)
            UNSUPPORTED_TYPES = {
                'zip', 'rar', '7z', 'tar', 'gz',
                'exe', 'dll', 'bin', 'iso', 'dmg',
                'db', 'sqlite', 'pkl', 'pyc'
                    ...
                    if ext == 'pdf':
                        ...
                        for slide in prs.slides:
                            for shape in slide.shapes:
                                if hasattr(shape, "text"): raw_text += shape.text + " "
                    elif ext in ['txt', 'md', 'csv', 'py', 'json', 'rtf', 'xml',
                                  'html', 'htm', 'yaml', 'yml', 'toml', 'ini', 'log']:
                        with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
                            raw_text = f.read(10000)
                    elif ext not in SUPPORTED_TEXT_TYPES:
                        unsupported_files.append(os.path.basename(filepath))
                        return ""
        ...
        # ── PART B: CNN image/video search (background thread) ─────────────
        if media_files:
    ...
    def handle_smart_organise(self):
        ...
        SUPPORTED_TEXT_TYPES_ORG = {
            'pdf', 'docx', 'txt', 'md', 'csv', 'py', 'json',
            'xlsx', 'xls', 'pptx', 'rtf', 'xml', 'html', 'htm',
            'yaml', 'yml', 'toml', 'ini', 'log'
        }
        # Excluding media from here since CNN handles them natively
        UNSUPPORTED_TYPES_ORG = {
            'zip', 'rar', '7z', 'tar', 'gz',
            'exe', 'dll', 'bin', 'iso', 'dmg',
            'db', 'sqlite', 'pkl', 'pyc'
        }
        ...
        def read_ultimate_precision_content(filepath):
            ...
            if ext in UNSUPPORTED_TYPES_ORG:
                unsupported_organise_files.append(file_name)
                return ""
            ...
            try:
                ...
                elif ext not in SUPPORTED_TEXT_TYPES_ORG:
                    unsupported_organise_files.append(file_name)
                    return ""
            except Exception as e:
        ...
        # Pre-check: warn about any directly selected text files that can't be read
        valid_text_files = []
        ...
        if unsupported_organise_files:
            QMessageBox.warning(self, "Unsupported File Types",
                f"The following {len(unsupported_organise_files)} file(s) cannot be analysed (no text content) and will be skipped:\n\n"
                + "\n".join(f"• {f}" for f in unsupported_organise_files)
                + "\n\nSupported types: PDF, DOCX, XLSX, PPTX, TXT, CSV, MD, PY, JSON, HTML, XML, YAML, LOG, etc.")
        ...
        try:
            # ── SINGLE TEXT FILE: Extract top keyword from content to name a new folder ──
            if len(files_selected) == 1 and not folders_selected and len(text_files) == 1:
                file_path = text_files[0]
                file_name = os.path.basename(file_path)

                text = read_ultimate_precision_content(file_path)
                if not text.strip():
                    QMessageBox.warning(self, "Cannot Read File",
                        f"'{file_name}' could not be read — it may have no text content (e.g. image-only PDF).\n\n"
                        "Try a different file format or ensure the file contains readable text.")
                    return

                progress = QProgressDialog(f"Analysing '{file_name}'...", None, 0, 0, self)
                ...
                # Use TF-IDF to extract the most meaningful keyword from the file content
                vectorizer = TfidfVectorizer(stop_words='english', max_features=500, ngram_range=(1, 2))
                X = vectorizer.fit_transform([text])
                feature_names = vectorizer.get_feature_names_out()
                scores = X.toarray()[0]

                # Pick top keyword as folder name, skip very short ones
                ...
                move_history.append((dest_file_path, file_path))
            ...
            # ── BRANCH A: 2+ folders, no files → similarity merge ────
            ...
            if folders_selected and files_selected:
                ...
                if image_files or video_files:
                    msg_res, cnn_m = self._classify_and_move_images(
                        image_files, video_files, dest_base=current_view_path, move_history=move_history,
                    )
                    results_message += msg_res.replace(results_message, "")  # prevent prefix duplication
                    metrics_message += cnn_m

                # B-2: Text Files → TF-IDF + RF + SVM
                if text_files:
            ...
            # ── BRANCH C: files only (or folders only fallback) → unsupervised ────────────
                ...
                if pool_images or pool_videos:
                    msg_res, cnn_m = self._classify_and_move_images(
                        pool_images, pool_videos, dest_base=current_view_path, move_history=move_history,
                    )
                    results_message += msg_res.replace(results_message, "") 
                    metrics_message += cnn_m

                # C-2: Text files → KMeans + RF + SVM
                if pool_text:
                    ...
                    if len(valid_files) >= 2:
                        ...
                        if n_clusters >= len(valid_files): n_clusters = max(2, len(valid_files) - 1)
    ...
    def handle_satisfaction_check(self, results, metrics, history, current_path):
        # Record Smart Organise usage every time it produces results
        if history:
        ...
        if history:
            ...
            # Right: Scrollable file results list
            right_panel = QWidget()
    ...
    def switch_view(self, identifier):
        # Get current language so the button translates instantly when swapped
    ...
    def handle_refresh(self):
        """Refreshes the current active view."""
    ...
    def _get_classifier(self):
        """Lazy-loads ImageClassifier on first call."""
    ...
    def _classify_and_move_images(self, image_paths: list, video_paths: list, dest_base: str, move_history: list, prefix_msg: str = "") -> tuple:
        ...
        progress = QProgressDialog(f"🖼️  CNN classifying {total} image/video file(s)…", "Cancel", 0, total, self)
...
class KemaslahApp(QMainWindow):
    ...
    def on_guest_exit(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            self._destroy_file_manager(); self.stack.setCurrentWidget(self.auth_window)
            self.auth_window.stack.setCurrentIndex(0)
            self.setMinimumSize(0, 0); self.setMaximumSize(16777215, 16777215)
            self.resize(1000, 750); self.showNormal()
    ...
    def on_logout(self):
        ...
        self.setMinimumSize(0, 0); self.setMaximumSize(16777215, 16777215)
        self.resize(1000, 750); self.showNormal(); self.auth_window.stack.setCurrentIndex(0)
