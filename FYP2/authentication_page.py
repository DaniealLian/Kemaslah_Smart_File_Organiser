...
# FIXED: Use deep_translator instead of googletrans for Python 3.13 compatibility
from deep_translator import GoogleTranslator

# Import updated database functions and mailer
...
class LoginPage(QWidget):
    def __init__(self, parent_stack, profile_page, language_page):
        ...
        # UI Elements stored as self for translation updates
    ...
    def handle_login(self):
        ...
        if not e or not p: 
            QMessageBox.warning(self, 
                translate_text("Validation Error", self.current_lang), 
                translate_text("All fields are required.", self.current_lang))
            return
        ...
        if user_data: 
            ...
            QMessageBox.critical(self, 
                translate_text("Login Failed", self.current_lang), 
                translate_text("Invalid Email or Password.", self.current_lang))
...
class ForgotPasswordPage(QWidget):
    ...
    def handle_otp_request(self):
        # ... existing logic ...
        pass # Keeping concise, assume logic remains
...
class RegisterPage(QWidget):
    ...
    def update_eye_icon(self, btn, is_hidden):
        icon_path = self.icon_hide if is_hidden else self.icon_visible
        if os.path.exists(icon_path): btn.setIcon(QIcon(icon_path)); btn.setIconSize(QSize(20, 20))
    def handle_email_verification(self): pass
    def poll_verification(self): pass
    def handle_registration(self): pass
...
class LanguagePage(QWidget):
    def __init__(self, parent_stack, login_page):
        ...
        self.logout_btn.clicked.connect(self.handle_logout) # FIXED: Connect to handle_logout
