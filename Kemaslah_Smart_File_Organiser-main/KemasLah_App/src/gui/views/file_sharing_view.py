...
# Replace your_username, your_password, and the cluster address.
...
class FileSharingView(QWidget):
    ...
    def sync_files(self):
        ...
        try:
            # --- NEW: Pull Sync List from MongoDB ---
            client = MongoClient(MONGO_URI)
            db = client["kemaslah_db"]
            rows = db.shared_files.find({"owner_email": self.user_email})
    ...
    def load_shared_files(self):
        try:
            # --- NEW: Pull Table Data from MongoDB ---
            client = MongoClient(MONGO_URI)
            db = client["kemaslah_db"]
    ...
    def on_double_click(self, row, col):
        ...
        # Unpack the data payload safely
        ...
        if direction == "Sent":
            ...
            # --- NEW: DOWNLOAD FILE FROM MONGODB CLOUD (The Backpack) ---
            try:
                client = MongoClient(MONGO_URI)
                db = client["kemaslah_db"]
                fs = gridfs.GridFS(db)
                
                # Search for the exact file in the MongoDB GridFS cluster
                cloud_file = fs.find_one({"filename": cloud_file_name})
                
                if cloud_file:
                    # If the file exists locally and is read-only, unlock it so we can overwrite it with the fresh cloud data
                    ...
                    # Download the bytes from the cloud and save them temporarily to the laptop
                    with open(path_to_open, 'wb') as f:
                        f.write(cloud_file.read())
                    print("File successfully downloaded from MongoDB Atlas!")
            except Exception as e:
                print(f"MongoDB Download Error: {e}")
                QMessageBox.warning(self, "Cloud Sync Warning", "Could not connect to MongoDB to fetch the latest file. Opening local copy if available.")
            # ------------------------------------------------------------
            ...
            if os.path.exists(path_to_open):
                ...
                if role_item:
                    ...
                    try:
                        # Re-apply the RBAC permission lock on the downloaded file
                        if role == "View":
