...
# --- UPDATED: FULL CLOUD COMMENT VIEWER ---
class CommentDialog(QDialog):
    """A custom viewer for reading and writing comments synced via MongoDB."""
    ...
    def save_comments(self):
        try:
            ...
            # --- NEW: UPLOAD COMMENT TO MONGODB ---
            client = MongoClient(MONGO_URI)
            db = client["kemaslah_db"]
            fs = gridfs.GridFS(db)

            # Clear old comments from cloud
            for old_c in fs.find({"filename": self.cloud_comment_name}):
                fs.delete(old_c._id)

            # Push new comment to cloud
...
class FileSharingView(QWidget):
    ...
    def sync_files(self):
        ...
        try:
            # --- NEW: TWO-WAY MONGODB SYNC LOGIC ---
            client = MongoClient(MONGO_URI)
            db = client["kemaslah_db"]
            fs = gridfs.GridFS(db)
            ...
            for row in rows:
                ...
                # Where is the file on THIS laptop?
                ...
                if cloud_file and os.path.exists(local_path):
                    ...
                    if difference > 5 and (is_owner or role == "Edit"):
                        ...
                        if reply == QMessageBox.StandardButton.Yes:
                            # Delete old cloud file & put new one
                    ...
                    # PULL: Cloud file is newer!
                        ...
                        if reply == QMessageBox.StandardButton.Yes:
                            ...
                            # Lock timestamps to prevent infinite loops
                            os.utime(local_path, (cloud_dt.timestamp(), cloud_dt.timestamp()))
                            sync_count += 1

                # Pull comments if they exist
                ...
                if cloud_comment:
                    # If we don't have it locally, or cloud is newer, download it
    ...
    def on_double_click(self, row, col):
        ...
        if direction == "Sent":
            ...
            # --- PULL LATEST COMMENTS FROM MONGODB FIRST ---
            try:
                client = MongoClient(MONGO_URI)
                db = client["kemaslah_db"]
                fs = gridfs.GridFS(db)
