...
# Replace DB_NAME = "kemaslah.db" with:
DB_NAME = DB_PATH
...
def share_file(file_path, owner_email, target_email, role, expiration_date):
    """Saves the file sharing permissions to the database."""
    try:
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO SharedFiles (file_path, owner_email, shared_with_email, role, expiration_date)
            VALUES (?, ?, ?, ?, ?)
        """, (file_path, owner_email, target_email, role, expiration_date))
        conn.commit()
        conn.close()
        return True
    except sqlite3.Error as e:
        print(f"Database error while sharing: {e}")
        return False

def revoke_file_share(file_path, owner_email, target_email):
    """Removes a sharing record from the database to revoke access."""
    try:
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        cursor.execute("""
            DELETE FROM SharedFiles 
            WHERE file_path = ? AND owner_email = ? AND shared_with_email = ?
        """, (file_path, owner_email, target_email))
        conn.commit()
        conn.close()
        return True
    except sqlite3.Error as e:
        print(f"Database error while revoking share: {e}")
        return False

def update_file_share(file_path, owner_email, target_email, new_role, new_expiration):
    """Updates an existing file sharing record with new roles or expiration dates."""
    try:
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        cursor.execute("""
            UPDATE SharedFiles 
            SET role = ?, expiration_date = ?
            WHERE file_path = ? AND owner_email = ? AND shared_with_email = ?
        """, (new_role, new_expiration, file_path, owner_email, target_email))
        conn.commit()
        conn.close()
        return True
    except sqlite3.Error as e:
        print(f"Database error while updating share: {e}")
        return False
